...
int main (int argc, char ** argv)
{
  ...
  // Hdf5Writer WR(outfile);   // ORIG: constructed on ALL ranks -> concurrent H5Fcreate
  // race -> corrupt 96-byte h5 -> FileIException abort. Boss-guard the writer (the sole
  // writer of the shared file), exactly like baryons_0000_dirac_claude.cc.
  ...
  for(int m=0;m<nmass;m++) {
    ...
    Meson5Trace_hdf(*FermActs[m],WR.get(),point_source  ,PointProps5[m]);
